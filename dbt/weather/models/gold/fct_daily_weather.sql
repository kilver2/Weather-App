with daily_weather as (
    select
        location_id,
        cast(forecast_time as date) as date_day,
...
all_data_historical as (
    ...
    left join daily_air_quality
        on
            ...
            and daily_weather.date_day = daily_air_quality.date_day
    left join {{ ref('stg_historical') }} as historical
        on
            daily_weather.location_id = historical.location_id
            and daily_weather.date_day = historical.date_day
),

all_data_historical_location_date as (
    select
        historical_all.location_id,
        historical_all.date_day,
    ...
    left join
        ...
        on historical_all.date_day = dim_date.date_day
)
